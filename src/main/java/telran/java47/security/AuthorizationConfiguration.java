    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests(authorize -> authorize
                ...
                .mvcMatchers(HttpMethod.POST, "/forum/post/{author}")
                    .access("#author == authentication.name and @customWebSecurity.checkPasswordExp(authentication.name)")
                .mvcMatchers(HttpMethod.PUT, "/forum/post/{id}/comment/{author}")
                    .access("#author == authentication.name and @customWebSecurity.checkPasswordExp(authentication.name)")
                .mvcMatchers(HttpMethod.PUT, "/forum/post/{id}")
                    .access("@customWebSecurity.checkPostAuthor(#id, authentication.name) and @customWebSecurity.checkPasswordExp(authentication.name)")
                .mvcMatchers(HttpMethod.DELETE, "/forum/post/{id}")
                    .access("@customWebSecurity.checkPostAuthor(#id, authentication.name) or hasRole(T(telran.java47.accounting.model.UserRole).MODERATOR and @customWebSecurity.checkPasswordExp(authentication.name)) and @customWebSecurity.checkPasswordExp(authentication.name)")
